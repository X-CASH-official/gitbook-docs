...
{% hint style="info" %}
If you installed the program through with [the autoinstaller](node-installation.md#quick-installation) and chosed to create a new block verifier key, you should have saved it at the end of the installation already. You don't need to generate a new one and can go[ register yourself as a delegate ](register-delegate.md#2-register-yourself-as-a-delegate)directly. 
{% endhint %}
...
```text
~/xcash-official/xcash-dpops/build/xcash-dpops --generate_key
```
...
{% hint style="info" %}
If you installed with the autoinstaller script, the wallet name will be **`xcash-dpops_WALLET`**
{% endhint %}
...
{% hint style="info" %}
If you installed the program through with [the autoinstaller](node-installation.md#quick-installation), your block verifier secret key has already been added as a paramaters to the **`xcash-dpops`** program. You can go to the next step.
{% endhint %}
...
ExecStart=~/xcash-official/xcash-dpops/build/xcash-dpops --block_verifiers_secret_key BLOCK_VERIFIER_SECRET_KEY --shared-delegates-website --fee <fee> --minimum_amount <amount>
Restart=always
